<?
	//include("inc_cabecera.php");
?>

		<h2>Cliente - Ver Informaci&oacute;n</h2>

			<div>
				<table class="zebra" width="95%" border="0" cellspacing="2" cellpadding="2">
				<thead>
				  <tr>
					<td width="25%">Campo</td>
					<td>Informaci&oacute;n</td>
				  </tr>
				 </thead>
				  
				 <tbody>
	
				<?
					foreach ($info_cliente->result() as $rowcli) {
				?>
				  <tr>
					<td>Razon Social</td>
					<td><h3><?=$rowcli->NOMBRE?></h3></td>
				  </tr>
				  <tr>
					<td>Tel&eacute;fono</td>
					<td><?=$rowcli->TELEFONO?></td>
				  </tr>
				  <tr>
				    <td>Tel&eacute;fono Celular: </td>
				    <td><?=$rowcli->CELULAR?></td>
			       </tr>
				  <tr>
				    <td>E-mail:</td>
				    <td><a href="mailto:<?=$rowcli->EMAIL?>"><?=$rowcli->EMAIL?></a></td>
			       </tr>
				  <tr>
				    <td>CUIT:</td>
				    <td><h4><?=$rowcli->CUIT?></h4></td>
			       </tr>
				  <tr>
				    <td>INGRESOS BRUTOS: </td>
				    <td><h4><?=$rowcli->INGRESOSBRUTOS?></h4></td>
			       </tr>
				  <tr>
				    <td>&nbsp;</td>
				    <td>&nbsp;</td>
			       </tr>
				  <tr>
				    <td valign="bottom"><a href="#" onclick=""></a></td>
				    <td align="right" valign="bottom"><a href="<?=site_url("clientes/ver/$rowcli->ID")?>"><img border="0" src="/gastos/system/application/views/images/boton_detalles.gif" alt="Ver Detalles" /></a></td>
			       </tr>
				<?
					}
				?>
				 </tbody>
			  </table>
			</div>
			
<?
	//include("inc_pie.php");
?>